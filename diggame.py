...
# todo :地下に行けば行くほど石が多くなったりとか鉱石とかが多くなったり耐久も上がったりさせたいなーって思ってます
...
SPRITE_BLOCK_ERROR = (SPRITE_BANK_BLOCK, 8, 0, 8, 8, 1)
SPRITE_BLOCK_DIRT = (SPRITE_BANK_BLOCK, 8, 0, 8, 8, 0)
SPRITE_BLOCK_GRASS = (SPRITE_BANK_BLOCK, 16, 0, 8, 8, 0)
SPRITE_BLOCK_STONE = (SPRITE_BANK_BLOCK, 8, 8, 8, 8, 0)
SPRITE_BLOCK_COAL = (SPRITE_BANK_BLOCK, 16, 8, 8, 8, 1)

SPRITE_BREAK_ANIM_BANK = SPRITE_BANK_BLOCK
...
class Block:
	...
	def __init__(self, x, y, world_seed_noise, world_seed_ore):
		...
		noise_val_hardness = px.noise(
									self.x * self.NOISE_SCALE_HARDNESS,
		...
		if self.y // BLOCK_SIZE == self.SURFACE_Y_LEVEL_IN_BLOCKS:
			...
			noise_val_ore = px.noise(
									self.x * self.NOISE_SCALE_ORE,
...
class GameMenu:
	...
	def draw(self, is_active):
		if is_active:
			...
			menu_title = "MENU"
			text_x_offset, text_y_offset = calculate_text_center_position(self.width, self.height, menu_title)
			px.text(self.x + text_x_offset, self.y + text_y_offset, menu_title, 0)
...
class DiggingGame:
	...
	def __init__(self):
		px.init(SCREEN_WIDTH, SCREEN_HEIGHT, title=self.GAME_TITLE, fps=60)

		try:
			px.load('sprite_sheet.pyxres')
			print("Successfully loaded sprite_sheet.pyxres.")
		except Exception as e:
			print(f"Warning: Failed to load 'sprite_sheet.pyxres': {e}")
	...
	def _create_dummy_sprites_if_needed(self):
		...
		img_bank0.rect(0, 8, 10, 10, px.COLOR_BLUE)
		...
		img_bank1.rect(16, 8, 8, 8, px.COLOR_DARK_GRAY)

		for i in range(5):
			img_bank1.rect(SPRITE_BREAK_ANIM_U, SPRITE_BREAK_ANIM_V_START + i * 8, 8, 8, px.COLOR_WHITE if i % 2 == 0 else px.COLOR_LIGHT_GRAY)
			img_bank1.text(SPRITE_BREAK_ANIM_U + 1, SPRITE_BREAK_ANIM_V_START + i * 8 + 1, str(i+1), px.COLOR_BLACK)
		px.images[SPRITE_BANK_BLOCK] = img_bank1
	...
	def _draw_ui_and_overlays(self):
		...
		self.game_menu.draw(self.is_menu_visible)
		...
		if self.show_debug_overlay:
			...
			px.text(2, 30, f"Coords:{len(self.generated_block_coordinates)}", 7)
