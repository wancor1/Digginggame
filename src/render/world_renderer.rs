...
impl WorldRenderer {
    fn get_background_info(
        ...
        let bx = (block.x / BLOCK_SIZE).floor() as i32;
        let by = (block.y / BLOCK_SIZE).floor() as i32;
    ...
    pub fn draw(game: &mut Game, atlas: Option<&Texture2D>) {
        ...
        for block in blocks {
            ...
            if block.y >= SURFACE_Y_LEVEL as f32 * BLOCK_SIZE {
            ...
            if !block.is_broken {
                if block.block_type.is_liquid() {
                    ...
                    if let (Some(rect), Some(atlas_tex)) = (block.sprite_rect, atlas) {
                        let level = block.liquid_level.clamp(1, 8) as f32;
